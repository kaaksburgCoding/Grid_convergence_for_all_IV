...
class Convergence1(object):
    ...
    def add_grids(self, grids):
        ...
        # calculate p and add grid ratios and p to a shared dictionary
        self._get_shared()

        # calculate extrapolated values, relative and extrapolated relative error and GCI
        # fine and coarse.
        self._get_fine_values()
        self._get_coarse_values()

        # aasemble GCI ratios and grid ratios
        self._get_ratios()

        # set up a grid for the report
        self._make_attributes()

        return

    # method for sorting and putting grid sizes and values in tuples
    # the grid is handed over to the method, its data will be used to create lists containing this data
    # WHERE DOES GRIDS COME FROM?
    def _set_grids(self, grids):

        #data type of grids, self._grids and pre_sort is "list"

        #create list for the grid data (empty until here)
        pre_sort = list(self._grids)
        
        #the grid data is put into the list
        pre_sort.extend(grids)
        #now the list is filled

        #the filled list is sorted by grid size (coarsest grid no. 1, finest grid no. 3)
        sorted_grids = sorted(pre_sort, key=lambda grid: grid[0])

        #the "self._grids" attribute of the class "Convergence" becomes a type tuple, not list anymore
    ...
    # method for making triplets:
    # [grid_number, grid_size, value]
    def _set_grid_triplets(self):
        ...
        # as long a there are values in the grid_list
        for grid, value in self._grids:

            # if absolute value of value > ~0, add grif number (1,2,3) to grid_list
            if abs(value) > self._zero_tol:
                numbered_grids.append([grid_num, grid, value])
                grid_num += 1

        # assemble list for new triplets
        grid_trips = []
    ...
    def _get_shared(self):
        ...
        for trip in self._grid_triplets:
            ...
            print("")
            print("function get_shared starts")
            print("grid_tiplets")
            print(self._grid_triplets)

            # Calculate the refinement ratios
            # taking grid size ([i][1]) out of triplets

            # r21: ratio between finest and middle grid
            # r32: ratio between middle and coarsest grid
    ...
    def _get_values(self, grid_one, grid_two, ratio, p):
        ...
        """
        print("")
        print("function get_values starts")
        ...
        try:
            print("p=",p)
            print("grid_one for calculate f_exact:")
            print(grid_one)
            print("grid_two for calculate f_exact:")
            print(grid_two)
            print("f_exact wird berechnet")
            f_exact = richardson_extrapolate(grid_one[2],
                                             grid_two[2],
                                             ratio,
                                             p)
            print("f_exact wurde berechnet")

        except ArithmeticError as e:
            print("Fehler bei f_exact-Berechnung")
            #warnings.warn(e)
            print("Warnung ausgegeben")
    ...
    def _get_ratios(self):
        ...
        # If the fine and coarse values are not calculated then they need to be
...
def main(out_path, analytical=None):
    # Read in the exel file and create a list with grid sizes and values
    comblist = assemble_cominations()

    main_list = assemble_cominations(exel_file)

    # Run convergence study
    # create a new instance of the the class Convergence (transfered parameter: analytical value if existent)
    convergence = Convergence(f_anal=analytical)

    # perform all actions listed und add_grids
...
""" method for creating a 2d-list (4x120) that contains all combinations of all measurement data for each mesurement point
    of the grid data for the next convergence loop out of the matrix with all simulation results.
    
    List Structure: 4 Rows for 4 measured datapoints    
                    120 columns for 120 possible combinations of different gridsizes as input for convergence study
    
    output of this method will be transfer parameter (datatype: list) for def main(): """

def assemble_cominations():

    #open csv file and create iterable numpy_array out of it (10x9)
    csvfile = open(r"C:\Users\Jose Matthias\Desktop\Uni\Bachelorarbeit_Inhalt\Simulations\examplecsvpythoninput_2.csv")
    ...
    # out of the numpy_array, create a 2d-tuplelist [4x10] that contains tuples of the grid size and the corresponding value
    tuple_list_grid_size_value = []

    for i in range (1,n_rows) :

        row_of_tuple_list_grid_size_value = []

        for j in range (n_columns) :

            new_tuple = (numpy_array[0][j],numpy_array[i][j])

            row_of_tuple_list_grid_size_value.append(new_tuple)

        tuple_list_grid_size_value.append(row_of_tuple_list_grid_size_value)

    #create a new empty list in which combinations will be safed
    combinations_list = []

    t_rows = len(tuple_list_grid_size_value)
    t_columns = len(tuple_list_grid_size_value[0])

    #loop over the rows of the numpy_array
    for k in range (t_rows) :

        #generate all possible combinations (120) and asseble them in a new list
        comb = list(combinations(tuple_list_grid_size_value[k], r=3))
...
def simple_read(file_name):
    ...
    # Create a new List[] called grid_list to add the grid sizes and values of the file
    grid_list = []
    ...
    for line in grid_file.readlines():
        ...
        # index the lines of the file (eg. ['1.0','0.97050'])
