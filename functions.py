...
# value_1: value for smallest grid
# value_2: value for middle grid
# value_3: value for coarsest grid

def order_of_convergence(value_1, value_2, value_3, ratio_21, ratio_32,
                         omega=0.5, tol=1.E-4, max_iter=1e3, exception_thrown=False):  # , start_p=1.):
    ...
    while abs(residual) > tol:
        ...
        if iterations > max_iter or residual > max_res:
            print("Residual out of range or too many iterations")
            return p1
...
def richardson_extrapolate(value_j, value_k, ratio_jk, p,exception_thrown_for_f_exact=False):
    #value_j: value of finer grid of the two
    #value_k: value of coarster grid of the two
...
"""  except:
        print("f_exact is too large")
        exception_thrown_for_f_exact=True

    if exception_thrown_for_f_exact:
        f_exact = 0 
 """
...
def gci(ratio_21, e21_approx, p):
    ...
    #to-do: In github:
    # r32: ratio of middle coarse to middle
    # r21: ratio of middle to fine
    # in my programm: other wy around
    #effect on gci_fine/gci_coarse falsch herum?
...
def asymptotic_ratio(gci_fine_21, gci_fine_32, ratio_21, p):
    ...
    asymp_ratio = ratio_21 ** p * (gci_fine_21 / gci_fine_32)
